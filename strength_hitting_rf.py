...
#set up plot saving
PLOT_FOLDER = r"C:\Users\gabi.reyes\Documents\Misc\open-biomech-driveline\results\strength_hitting_rf"
...
hp_correlation = hp_df[hp_numeric_cols + ['bat_speed_mph']].corr()['bat_speed_mph'].sort_values(ascending=False)
top_10_correlations = pd.DataFrame({
    'Feature': hp_correlation.iloc[1:21].index,
    'Correlation': hp_correlation.iloc[1:21].values
})

print("\nTop strength correlations with bat speed:")
...
hp_df = hp_df.dropna(subset=['bat_speed_mph'])

features = [col for col in hp_df.columns if col not in ['bat_speed_mph', 'athlete_uid', 'bat_speed_mph_group'] + date_columns]

X = hp_df[features]
y = hp_df['bat_speed_mph']
...
top_features_df = hp_df[['bat_speed_mph'] + top_10_features]
correlation_matrix = top_features_df.corr()
plt.figure(figsize=(12, 10))
sns.heatmap(correlation_matrix, annot=True, cmap='coolwarm', vmin=-1, vmax=1, center=0)
plt.title('Correlation Heatmap of Top 10 Features with Bat Speed')
plt.tight_layout()
...
def linear_regression_analysis(feature, df):
    ...
    correlation = df[feature].corr(df['bat_speed_mph'])

    fig, ax = plt.subplots(figsize=(10, 6))
    sns.regplot(x=feature, y='bat_speed_mph', data=df, scatter_kws={'color': 'blue','alpha':0.5}, line_kws={'color':'red'}, ax=ax)
    ax.set_xlabel(feature)
    ax.set_ylabel('Bat Speed (mph)')
    ax.set_title(f'Linear Regression: Bat Speed vs {feature}')
    equation = f"Bat Speed = {lr_model.intercept_:.2f} + {lr_model.coef_[0]:.2f} * {feature}"
...
df = hp_df[top_10_features + ['bat_speed_mph']].copy()
df = df.dropna()
results = []
